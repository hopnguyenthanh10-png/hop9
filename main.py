...
async def cb_handler(e):
    ...
    # ==================== MENU ADMIN ====================
        ...
        for c in cats:
            txt += f"🔸 ID {c['id']}: {c['name']} | Giá: {c['price']}đ | Bot: @{c['target_bot']}\n"
        btns = [
            [TButton.inline("➕ THÊM MỚI", b"add_cat")],
    ...
    # ==================== MENU KHÁCH HÀNG ====================
        ...
        btns = [[TButton.inline(f"🎮 {c['name']} - {c['price']:,}đ", f"vcat_{c['id']}")] for c in cats]
        btns.append([TButton.inline("🔙 QUAY LẠI", b"back")])
        await e.edit("🛒 **CHỌN LOẠI GAME MUỐN MUA:**", buttons=btns)
        ...
        btns = [
            [TButton.inline(f"🛒 Mua 1 Code", f"buy_{cid}_1")],
